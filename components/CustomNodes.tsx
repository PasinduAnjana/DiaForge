...
export const DatabaseNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    <div 
      ...
      className={`relative w-28 h-32 flex flex-col items-center justify-center font-sans transition-all ${selected ? 'drop-shadow-[0_0_8px_rgba(59,130,246,0.4)]' : 'drop-shadow-md'}`}
    >
...
export const ServerNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    <div 
      onDoubleClick={() => setIsEditing(true)}
      className={`px-4 py-2 shadow-md rounded-md bg-white dark:bg-zinc-900 border-2 ${selected ? 'border-green-500' : 'border-zinc-300 dark:border-zinc-700'} text-zinc-800 dark:text-zinc-100 flex items-center gap-2 min-w-[150px] transition-colors`}
    >
...
export const CustomNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    <div 
      onDoubleClick={() => setIsEditing(true)}
      className={`w-24 h-24 shadow-md rounded-md bg-white dark:bg-zinc-900 border-2 ${selected ? 'border-purple-500' : 'border-zinc-300 dark:border-zinc-700'} text-zinc-800 dark:text-zinc-100 flex flex-col items-center justify-center gap-2 transition-colors p-2`}
    >
      ...
      <div className="w-full flex justify-center">
        <EditableLabel
          ...
          isEditing={isEditing}
          setIsEditing={setIsEditing}
          className="text-xs text-zinc-500 dark:text-zinc-400 text-center w-full"
